...
if db_args_new['password']:
    paswd = bytes(db_args_new.pop('password', None), encoding='UTF-8')
    keyring.set_password('cats_service', 'user', paswd)

if not os.path.exists(f'{PATH}/config.json'):
    with open(f'{PATH}/config.json', 'w', encoding='UTF-8') as config:
        config.write(str(db_args_default))
